 ...
 */
export const sendRideNotification = functions.firestore
    ...
    .onCreate(async (snap, context) => {
        ...
        const usersToNotify = [creatorId, ...participantIds].filter(
            (id: string) => id !== joinerId
        );
        ...
        try {
            ...
            if (response.failureCount > 0) {
                response.responses.forEach((resp, idx) => {
                    if (!resp.success) {
                        console.error(
                            `Failed to send to token ${tokensToNotify[idx]}: `,
                            resp.error
                        );
